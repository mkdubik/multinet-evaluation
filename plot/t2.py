...
def main():
	...
	b = plot(data, 'lart', 0, 1000)
	c = plot(data, 'pmm', 0, 1000)
	...
	plt.xticks(x)
	plt.savefig('plot/t2a')

	plt.figure(2)
	a = plot(data, 'glouvain', 1, 1000)
	b = plot(data, 'lart', 1, 1000)
	c = plot(data, 'pmm', 1, 1000)

	plt.xlabel('Layers', fontsize = 14)
	plt.ylabel('memory usage (megabytes)', fontsize = 14)
	plt.legend(handles = [a, b, c], loc = 2, title = 'N = 1000, $\mu$ = 0.3')
	plt.xticks(x)
	plt.savefig('plot/t2b')

	'''
	plt.figure(3)
	a = plot(data, 'glouvain', 0, 4000)
	b = plot(data, 'lart', 0, 4000)
	c = plot(data, 'pmm', 0, 4000)
	...
	b = plot(data, 'lart', 1, 4000)
	c = plot(data, 'pmm', 1, 4000)
